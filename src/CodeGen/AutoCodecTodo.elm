module CodeGen.AutoCodecTodo exposing (AutoCodecTodo, ProjectContext, codecTypeTodoFixes, declarationVisitorGetCodecs, getTodos, todoErrors)
    ...
    case ( function.signature, function.declaration ) of
        ( Just (Node _ signature), Node _ declaration ) ->
            case Helpers.typeAnnotationReturnValue signature.typeAnnotation of
                Node _ (TypeAnnotation.Typed (Node _ ( [], "AutoCodec" )) [ Node _ (TypeAnnotation.Typed codecType _) ]) ->
...
getTodos context declarationRange function =
    ...
    case function.signature of
        Just (Node _ signature) ->
            case Helpers.typeAnnotationReturnValue signature.typeAnnotation of
                Node _ (TypeAnnotation.Typed (Node _ ( [], "AutoCodec" )) [ Node _ _, Node _ (TypeAnnotation.Typed codecType _) ]) ->
                    abc signature codecType

                Node _ (TypeAnnotation.Typed (Node _ ( [], "AutoCodec" )) [ Node _ (TypeAnnotation.Typed codecType _) ]) ->
                    abc signature codecType

                Node _ (TypeAnnotation.Typed (Node _ ( [ "AutoCodec" ], "AutoCodec" )) [ Node _ _, Node _ (TypeAnnotation.Typed codecType _) ]) ->
                    abc signature codecType

                Node _ (TypeAnnotation.Typed (Node _ ( [ "AutoCodec" ], "AutoCodec" )) [ Node _ (TypeAnnotation.Typed codecType _) ]) ->
                    abc signature codecType
    ...
    List.foldl
        (\( fieldName, typeAnnotation ) ( code, imports ) ->
            ...
            ( code
                |> Helpers.pipeRight
                    (Helpers.application
                        [ Helpers.functionOrValue [ "AutoCodec" ] "field"
        ...
        ( Helpers.application
            [ Helpers.functionOrValue [ "AutoCodec" ] "object"
        ...
        recordFields
        |> Tuple.mapFirst (Helpers.pipeRight (Helpers.application [ Helpers.functionOrValue [ "AutoCodec" ] "buildObject" ]))
...
codecFromTypeAnnotation projectContext existingImports currentModule typeAnnotation =
    case typeAnnotation of
        Typed_ qualifiedType typeVariables ->
            let
                ...
                getCodecName text =
                    case text of
                        "Int" ->
                            Helpers.functionOrValue [ "AutoCodec" ] "int"

                        "Float" ->
                            Helpers.functionOrValue [ "AutoCodec" ] "float"

                        "String" ->
                            Helpers.functionOrValue [ "AutoCodec" ] "string"

                        "Bool" ->
                            Helpers.functionOrValue [ "AutoCodec" ] "bool"

                        "Maybe" ->
                            Helpers.functionOrValue [ "AutoCodec" ] "nullable"

                        "Dict" ->
                            Helpers.functionOrValue [ "AutoCodec" ] "dict"

                        "Set" ->
                            Helpers.functionOrValue [ "AutoCodec" ] "set"

                        "Result" ->
                            Helpers.functionOrValue [ "AutoCodec" ] "result"

                        "List" ->
                            Helpers.functionOrValue [ "AutoCodec" ] "list"

                        "Array" ->
                            Helpers.functionOrValue [ "AutoCodec" ] "array"
                ...
                applied =
                    ( case Helpers.find (.typeVar >> (==) qualifiedType) projectContext.autoCodecs of
                        Just codec ->
        ...
        Unit_ ->
            ( Helpers.functionOrValue [ "AutoCodec" ] "unit", Set.empty )
        ...
        Tupled_ [ first, second ] ->
            ...
            in
            ( Helpers.application
                [ Helpers.functionOrValue [ "AutoCodec" ] "tuple"
                , codec0
                , codec1
                ]
                |> Helpers.parenthesis
        ...
        Tupled_ [ first, second, third ] ->
            ...
            in
            ( Helpers.application
                [ Helpers.functionOrValue [ "AutoCodec" ] "triple"
                , codec0
                , codec1
                , codec2
                ]
...
generateCustomTypeCodec projectContext currentModule existingImports customType =
    let
        ...
        start =
            ( Helpers.application
                [ Helpers.functionOrValue [ "AutoCodec" ] "custom"
                , Expression.LambdaExpression
    ...
    List.foldl
        (\constructor ( code, imports ) ->
            ...
            ( Helpers.pipeRight
                (Helpers.application
                    (Helpers.functionOrValue [ "AutoCodec" ]
        ...
        |> (\( expression, imports ) ->
                ( Helpers.pipeRight
                    (Helpers.functionOrValue [ "AutoCodec" ] "buildCustom")
                    expression
                , imports
                )
           )
    ...
    getCodecTypes projectContext
        ...
        |> List.concatMap
            (\( currentModule, qualifiedTypes ) ->
                let
                    todosInModule : List AutoCodecTodo
                    todosInModule =
                        List.filterMap
                            (\( moduleName_, todo ) ->
                ...
                Set.toList qualifiedTypes
                    ...
                    |> List.filter (\a -> List.all (.typeVar >> (/=) a) projectContext.autoCodecs)
                    |> List.indexedMap
                        (\index qualifiedType ->
                            let
                                maybeType : Maybe ( ModuleName, TypeOrTypeAlias )
